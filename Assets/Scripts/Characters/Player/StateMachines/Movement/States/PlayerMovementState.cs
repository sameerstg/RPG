...
*/        if (enemy && Vector3.Distance(enemy.transform.position,stateMachine.Player.transform.position)<=stateMachine.abilites.attackingRange)
    ...
    float GetDirectionAngle(Vector3 direction)
        ...
        {
            float movementSpeed = abilities.speed.value * stateMachine.ReusableData.MovementSpeedModifier;
        ...
        protected void EnableCameraRecentering(float waitTime = -1f, float recenteringTime = -1f)
        {
            ...
            if (movementSpeed == 0f)
            {
                movementSpeed = abilities.speed.value;
            }

            stateMachine.Player.CameraRecenteringUtility.EnableRecentering(waitTime, recenteringTime, abilities.speed.value, movementSpeed);
        }
